...
from services.subsystem_service import get_subsystems, filter_subsystems_by_title, get_subsystems_false
...
@router.get("/status_false/", response_model=List[SubsystemResponse])
async def read_subsystems_status_false(db: AsyncSession = Depends(get_db)):
    return await get_subsystems_false(db)
